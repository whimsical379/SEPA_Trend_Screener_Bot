...
## 1. 项目简介
本项目是一个基于 Python 开发的自动化股票筛选系统，核心逻辑严格遵循传奇交易大师 **Mark Minervini** 的 **SEPA (Specific Entry Point Analysis)** 趋势模板。

### 支持市场
- **美股**：每日北京时间 06:00 自动筛选
- **A 股**：每日北京时间 16:30 自动筛选（v1.5 新增）

### v1.5 更新内容 (2026-09-01)

**新增功能：**
* **A 股筛选版本**：新增 `sepa_a_share_bot.py`，使用 akshare 作为数据源，支持 A 股市场完整筛选流程。
* 新增 `config_a_share.yaml`：A 股专属策略参数（市值 50亿起、5% 止损、沪深 300 基准、RSI 72 阈值）。
* 新增 `.github/workflows/run_a_share.yml`：A 股独立定时任务，北京时间 16:30 自动运行。
* A 股适配：排除 ST 股、前复权数据处理、沪深两市交易日历（exchange_calendars XSHG）。
* 两个市场共用 SMTP 邮件推送配置，邮件主题自动区分"（美股）"/"（A股）"。
...
| 参数 | 美股 | A 股 | 调整原因 |
|:---|:---|:---|:---|
| 市值范围 | $50亿 ~ $2000亿 | ¥50亿 ~ ¥10000亿 | A 股市值分布不同 |
| 止损位 | 7% | 5% | T+1 风险，收紧止损 |
| 保本位 | 14% | 10% | 同步收紧 |
| RSI 上限 | 70 | 72 | A 股散户多，RSI 易钝化 |
| 放量倍数 | 1.0x | 1.3x | A 股量能信号更重要 |
| 最大推荐数 | 3 | 3 | 保持一致 |
